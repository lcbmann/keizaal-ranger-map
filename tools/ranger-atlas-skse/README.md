...
The `0.7.10` build initializes as an SKSE DLL and waits for SKSE's
...
The `0.7.10` build adds an in-game Ranger Atlas action menu after the character has
entered the world:
...
The `0.7.10` release moves native marker creation out of the C++ DLL. The
...
The `0.7.10` diagnostic build writes Platform lifecycle messages through
